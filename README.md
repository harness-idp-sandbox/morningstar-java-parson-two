...
# run app
java -jar target/java-parson-two-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-parson-two:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-parson-two -n dev

# get external endpoint
kubectl get svc java-parson-two-svc -n dev
```
...
```bash
kubectl port-forward svc/java-parson-two-svc 8080:80 -n dev
...
kubectl rollout restart deploy/java-parson-two -n dev
kubectl rollout status deploy/java-parson-two -n dev
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javaparsontwo/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javaparsontwo/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
